...
# SCHEDULE_START   = ""
RUN_DURATION_MIN = 150   # e.g. 180 for 3 hours, or None for "run until stopped"
...
# BASE_NAME = "Under-19s Asia Cup 202526 M05 IND19 v PAK19 HLTS SONY TEN1 IPTV1"
...
def maybe_accept_chunk(chunk_path, chunk_name, context):
    ...
    min_expected_br = nm3u8dl_selected_bitrate * 0.5 if nm3u8dl_selected_bitrate > 0 else 500
    if dur > STABILITY_SEC and br > min_expected_br:
...
def make_black_clip():
    ...
    name = f"black_{bw}x{bh}_{bfps}fps_2s.ts"
    ...
    cmd = [
        ...
        "-y",
        "-f", "lavfi",
        "-i", f"color=black:s={bw}x{bh}:r={bfps}:d=2",
        "-c:v", "libx264",
        "-preset", "veryfast",
        "-pix_fmt", "yuv420p",
        "-f", "mpegts",
...
def finalize_recording():
    ...
    #proc = subprocess.Popen(cmd, cwd=CHUNKS_DIR)
    #while proc.poll() is None:
    #    if stop_flag:
    #        proc.terminate()
    #        break
    ...
    try:
        ...
        lines.append("Final file duration     : " + fmt_hms(dur))
        ...
        black_len = 2.0         # length of black separator clip you generate
        for idx, sess in enumerate(sessions, start=1):
            start_ts = sess["start"]
            dur_media = sess.get("dur_media", 0.0)
            end_ts = start_ts + dur_media
            ...
            lines.append(f"  Recorded for     : {fmt_hms(dur_media)}  (segment)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        FINAL_FILE = f"{BASE_NAME}_{run_ts}.mp4"
